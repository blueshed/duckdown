...
| `image` | The picture on the card a shared link shows (`og:image`): `images/cover.jpg` (under `static/`), a site path (`/static/images/cover.jpg`) or a full URL. An item's own picture is its card without saying so; an each: page may name another (`image: {{item-detail}}`). A page with none shares the site's: `static/card.jpg` (1200×630, made in the editor's Resources → icon), else the home-screen icon, small |
...
- **A picture is kept at narrower widths too.** Uploading a JPEG, PNG or WebP (here, or into a collection) also writes it at 480, 960 and 1600 pixels wide beside it (`photo-960w.jpg`), never wider than the original and only when the copy is smaller in bytes, and notes them in the folder's hidden `.widths.json`. Every `<img>` of it under `/static/images/` — a markdown picture, an item's picture — then gets a `srcset`, so a phone downloads a width, not the original. The grid doesn't list the widths. For pictures already there, or written by hand, run `duckdown images` (for a served site, against its bucket: `railway run --service <name> bunx duckdown images`) — then restart the served site, which keeps what it has read of each folder until a change made through it (an upload, a save) or a restart. SVG needs none; a picture outside `static/images/` (another host, a bucket of originals) gets none.
...
- **Resources** (a drawer over the preview, from the header): what a page is composed with, in three tabs, the site's icon, and a fifth to read.
  ...
  - A visitor report for a site whose server logs views (`DUCKDOWN_LOG=1`): feed its log to `duckdown report` — piped (`railway logs | duckdown report`) or as a saved file (`duckdown report site.log`) — and it writes `reports/<month>/<day>.md` for each day the lines cover, and the month's `index.md` — each day linked, and the month's totals — made again from the days each time: views by readers and by crawlers, the most read pages, addresses that weren't there (worth an `aliases:` line), and the sites that sent readers. Scanners' probes — `/.env`, `/.git/config`, `/wp-login.php` and the like, which pose as browsers — are counted in a line of their own and kept out of the rest; so are the files a page pulls in (pictures, stylesheets, the search index), and a certificate authority's checks count as a crawler's. It counts exactly the lines it is given, and names no reader; give it whole days, since a day it reports again is replaced.
